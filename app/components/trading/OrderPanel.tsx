...
export default function OrderPanel({onNewOrder, symbol}: OrderPanelProps) {
    ...
    // 주문 처리 함수
    const handleOrder = async (orderType: '매수' | '매도') => {
        if (quantity <= 0) {
        ...
        try {
            ...
            if (!response.ok) {
                // 서버가 보낸 에러 메시지를 파싱하여 보여줄 수 있습니다.
        ...
        <div className="bg-white rounded-lg shadow-sm">
            ...
            <div className="p-4">
                {message && ( // 메시지가 있을 경우 표시
                    <div
                        className="bg-yellow-100 border border-yellow-400 text-yellow-700 px-4 py-2 rounded relative mb-4"
                        role="alert">
                        <span className="block sm:inline">{message}</span>
                        <span className="absolute top-0 bottom-0 right-0 px-4 py-2" onClick={() => setMessage(null)}>
              <svg className="fill-current h-6 w-6 text-yellow-500" role="button" xmlns="http://www.w3.org/2000/svg"
                            ...
                            <table className="w-full text-sm text-left">
                                ...
                                {safeOrders.length > 0 ? (
                                    safeOrders.slice(0, 15).map((order, idx) => ( // 최근 15개만 표시
                                        <tr key={idx}
                                            className={`align-middle ${order.type === '매수' ? 'text-red-600' : 'text-blue-600'}`}>
                                            ...
                                            {/* 수량 소수점 4-8자리 */}
                                            <td className="text-right">{formatPriceKRW(order.total)}</td>
                                            {/* 총액 정수 표시 */}
                                        </tr>
                                    ))
                                ) : (
                            ...
                            <div>
                                <div className="flex items-center gap-4 mb-3">
                                    ...
                                    <div className="flex gap-2 text-xs">
                                        <label className="flex items-center gap-1"><input type="radio" name="orderType"
                                                                                          defaultChecked/> 지정가</label>
                                        <label className="flex items-center gap-1"><input type="radio"
                                                                                          name="orderType"/> 시장가</label>
                                    </div>
                                </div>
                                <div className="space-y-3">
                                    <div className="grid grid-cols-3 items-center">
                                        ...
                                        <p className="col-span-2 text-right font-semibold">
                                            {activeTab === '매수' ? `${formatPriceKRW(krwBalance)} KRW` : `${formatQuantityBTC(btcBalance)} 주`}
                                        </p>
                                    </div>
                                    <div className="grid grid-cols-3 items-center">
                                        <label className="font-semibold text-gray-600">주문가격</label>
                                        <div className="col-span-2 flex items-center border rounded-md">
                                            <button className="px-2 text-lg text-gray-400"
                                ...
                                {activeTab === '매수' && <button onClick={() => handleOrder('매수')}
                                                               className="w-full mt-4 py-3 bg-red-500 text-white font-bold rounded-lg hover:bg-red-600">매수</button>}
                                {activeTab === '매도' && <button onClick={() => handleOrder('매도')}
                                                               className="w-full mt-4 py-3 bg-blue-500 text-white font-bold rounded-lg hover:bg-blue-600">매도</button>}
                            </div>
                        ) : activeTab === '간편주문' ? (
                            // 간편주문 탭 내용
                            <div className="space-y-4">
                                <div className="flex justify-between items-center">
                                    ...
                                    <button className="px-4 py-2 border rounded text-red-500 font-semibold"
                                            onClick={() => handleOrder('매수')}>매수
                                    </button>
                                </div>
                                <div className="flex justify-between items-center">
                                    <p className="font-semibold">시장가 전액 매도</p>
                                    <button className="px-4 py-2 border rounded text-blue-500 font-semibold"
                                            onClick={() => handleOrder('매도')}>매도
                                    </button>
                                </div>
                                <div className="flex justify-between items-center"><p className="font-semibold">지정가 전액
                                    매수</p>
                                ...
                                <div className="flex justify-between items-center"><p className="font-semibold">일반 주문
                                    취소</p>
                        ...
                        {activeTab === '매수' || activeTab === '매도' ? (
                            <div>
                                ...
                                <div className="overflow-auto max-h-64">
                                    <table className="w-full text-xs">
                                        <thead>
                                        <tr className="text-gray-500">
                                            <th className="text-left font-normal">시간</th>
                                            <th className="text-right font-normal">가격</th>
                                        ...
                                        {safeOrders.slice(0, 15).map((order, idx) => ( // 최근 15개만 표시
                                            <tr key={idx}
                                                className={order.type === '매수' ? 'text-red-500' : 'text-blue-500'}>
                            ...
                            <div>
                                <div className="flex border-b mb-2 text-sm">
                                    <button onClick={() => setSimpleView('depth')}
                                            className={`flex-1 py-2 ${simpleView === 'depth' ? 'border-b-2 border-gray-800 font-bold' : 'text-gray-500'}`}>마켓뎁스
                                    </button>
                                    <button onClick={() => setSimpleView('chart')}
                                            className={`flex-1 py-2 ${simpleView === 'chart' ? 'border-b-2 border-gray-800 font-bold' : 'text-gray-500'}`}>미니차트
                                    </button>
                                </div>
                                {simpleView === 'depth' ?
                                    <div className="h-full bg-gray-50 rounded p-4 flex items-center justify-center"><p
                                        className="text-gray-400">[Market Depth Placeholder]</p></div> :
                                    <div className="h-full bg-gray-50 rounded p-4 flex items-center justify-center"><p
                                        className="text-gray-400">[Mini Chart Placeholder]</p></div>}
                            </div>
                        ) : null}
                    </div>
                )}
